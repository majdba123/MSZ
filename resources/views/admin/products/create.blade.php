    ...
    <form id="create-form" class="space-y-6" novalidate enctype="multipart/form-data">
        ...
        <div class="card">
            <div class="card-body border-b border-gray-100">
                ...
                <p class="mt-0.5 text-sm text-gray-500">Enter product information below.</p>
            </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                {{-- Drop Zone --}}
                <div id="drop-zone" class="group flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-gray-300 bg-gray-50/50 px-6 py-10 text-center transition-colors hover:border-brand-400 hover:bg-brand-50/30">
                    <div class="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-brand-100 text-brand-600 transition-transform group-hover:scale-110">
                        <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909M3.75 21h16.5A2.25 2.25 0 0022.5 18.75V5.25A2.25 2.25 0 0020.25 3H3.75A2.25 2.25 0 001.5 5.25v13.5A2.25 2.25 0 003.75 21z"/></svg>
                    </div>
                    <p class="text-sm font-medium text-gray-700">Drag & drop images here, or <span class="text-brand-600 underline">browse</span></p>
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    // Drag & drop
    ['dragenter', 'dragover'].forEach(e => dropZone.addEventListener(e, ev => { ev.preventDefault(); dropZone.classList.add('border-brand-400', 'bg-brand-50/40'); }));
    ['dragleave', 'drop'].forEach(e => dropZone.addEventListener(e, ev => { ev.preventDefault(); dropZone.classList.remove('border-brand-400', 'bg-brand-50/40'); }));
    ...
    function renderPreviews() {
        if (selectedFiles.length === 0) { preview.innerHTML = ''; countEl.classList.add('hidden'); return; }
