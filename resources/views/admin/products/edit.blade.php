...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    function renderExistingPhotos() {
        ...
        container.innerHTML = existingPhotos.map(photo => {
            ...
            return `<div class="group relative aspect-square overflow-hidden rounded-lg border-2 transition-colors ${selectedIds.has(photo.id) ? 'border-red-400 ring-2 ring-red-200' : isPrimary ? 'border-blue-400 ring-2 ring-blue-200' : 'border-gray-200'}">
    ...
    window.setPrimaryPhoto = async function (photoId) {
        try {
            await window.axios.patch(`/api/admin/products/${productId}/photos/${photoId}/set-primary`);
