    ...
    try {
        ...
        $('subs-grid').innerHTML = subs.map(s => `
            <a href="/subcategories/${s.id}" class="group flex items-center gap-3 rounded-2xl border border-gray-200/80 bg-white p-4 transition-all duration-200 hover:-translate-y-1 hover:shadow-lg dark:border-gray-800 dark:bg-gray-900">
                <div class="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-gray-100 dark:bg-gray-800">
                    ${s.image ? `<img src="/storage/${esc(s.image)}" class="h-full w-full object-cover" alt="">` : `<svg class="h-5 w-5 text-gray-300 dark:text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159"/></svg>`}
                </div>
