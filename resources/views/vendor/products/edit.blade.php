...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    function renderExistingPhotos() {
        ...
        container.innerHTML = existingPhotos.map(photo => `
            <div class="group relative aspect-square overflow-hidden rounded-lg border-2 transition-colors ${selectedIds.has(photo.id) ? 'border-red-400 ring-2 ring-red-200' : 'border-gray-200'}" id="photo-${photo.id}">
    ...
    window.removeSelectedPhotos = async function () {
        ...
        try {
            await window.axios.delete('/api/vendor/products/' + productId + '/photos', {
    ...
    window.uploadNewPhotos = async function () {
        ...
        try {
            const res = await window.axios.post('/api/vendor/products/' + productId + '/photos', formData, {
    ...
    form.addEventListener('submit', async function (e) {
        ...
        try {
            await window.axios.put('/api/vendor/products/' + productId, {
