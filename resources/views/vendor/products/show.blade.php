...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    try {
        ...
        document.getElementById('product-discount-value').textContent = p.discount_percentage ? parseFloat(p.discount_percentage).toFixed(2) + '%' : 'No discount';
        document.getElementById('product-discount-start').textContent = p.discount_starts_at ? new Date(p.discount_starts_at).toLocaleString() : '—';
        document.getElementById('product-discount-end').textContent = p.discount_ends_at ? new Date(p.discount_ends_at).toLocaleString() : '—';
        updateDiscountStatusDisplay(p.discount_status);
        ...
        function updateDiscountStatusDisplay(status) {
            ...
            if (status === 'cancelled') {
                el.innerHTML = '<span class="badge badge-danger">Cancelled</span>';
                return;
            }
        ...
        if (primaryPhoto) {
            primaryPhotoContainer.innerHTML = `
                <div class="group relative w-full max-w-2xl overflow-hidden rounded-xl border border-emerald-200 bg-gradient-to-br from-gray-50 to-gray-100 p-4 ring-1 ring-emerald-100">
                    ...
                    <div class="absolute left-6 top-6 rounded bg-emerald-600 px-2 py-1 text-xs font-semibold text-white">Primary Photo</div>
                    <div class="absolute inset-0 flex items-center justify-center gap-2 bg-black/0 transition-all group-hover:bg-black/50 group-hover:opacity-100">
                        <button type="button" onclick="viewPhotoLarge('${primaryPhoto.url}')" title="View Large" class="flex h-10 w-10 items-center justify-center rounded-full bg-emerald-600 text-white opacity-0 shadow-lg transition-all hover:bg-emerald-700 group-hover:opacity-100">
        ...
        if (otherPhotos.length === 0) {
            ...
            photosContainer.innerHTML = otherPhotos.map(photo => {
                return `<div class="group relative aspect-[4/3] overflow-hidden rounded-lg border border-gray-200 bg-gradient-to-br from-gray-50 to-gray-100 p-2 transition-colors sm:aspect-square">
                    ...
                    <div class="absolute inset-0 flex items-center justify-center gap-2 bg-black/0 transition-all group-hover:bg-black/50 group-hover:opacity-100">
                        <button type="button" onclick="viewPhotoLarge('${photo.url}')" title="View Large" class="flex h-8 w-8 items-center justify-center rounded-full bg-emerald-600 text-white opacity-0 shadow-lg transition-all hover:bg-emerald-700 group-hover:opacity-100">
